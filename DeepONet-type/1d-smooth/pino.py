...
class PINO_loss:
    def __init__(self, eps, batch_size, N, coeffs, myloss, device):
        x_left = torch.linspace(0, 0.5, N//2+1).to(device)
        x_right = torch.linspace(0.5, 1, N//2+1).to(device)
        self.q_left = 1+torch.exp(x_left)
        self.q_right =  1-torch.log(x_right+1)
...
if __name__ == '__main__':
    ...
    f = np.load('DeepONet-type/1d-smooth/f.npy')
    ...
    pino_loss = PINO_loss(1., batch_size, N, coeffs, myloss, device)
    model_left.train()
    model_right.train()
    ...
    N_fine = 256
    f_test_fine = np.load('DeepONet-type/1d-smooth/f_test_fine.npy')
    u_test_fine = np.load('DeepONet-type/1d-smooth/u_test.npy')
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig('DeepONet-type/1d-smooth/1d_fno_example.png')