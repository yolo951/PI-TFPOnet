...
def PINO_loss(u_left, fx_left, u_right, fx_right, myloss, coeffs):
    ...
    grads = FDM(u_left, u_right, eps=0.001)
...
if __name__ == '__main__':
    ...
    scheduler_left = torch.optim.lr_scheduler.MultiStepLR(optimizer_left, milestones=[1500, 3500, 7000, 10000, 15000], gamma=gamma)
    scheduler_right = torch.optim.lr_scheduler.MultiStepLR(optimizer_right, milestones=[1500, 3500, 7000, 10000, 15000], gamma=gamma)
    mse_history = []
    rel_l2_history = []
    model_left.train()
    model_right.train()
    ...
    model_left.eval()
    model_right.eval()
    np.save('DeepONet-type/2d-singular/saved_data/unsupervised_fno_loss_history.npy', mse_history)
    np.save('DeepONet-type/2d-singular/saved_data/unsupervised_fno_rel_l2_history.npy', rel_l2_history)
    torch.save({'model_left': model_left.state_dict(), 'model_right': model_right.state_dict()}, 'DeepONet-type/2d-singular/saved_data/unsupervised_fno_model_state.pth')
    ...
    # plt.ylim(1e-3, 1e+2)
    plt.yscale("log")
    plt.savefig('DeepONet-type/2d-singular/saved_data/unsupervised_fno_l2.png')
    ...
    plt.tight_layout()
    plt.savefig('DeepONet-type/2d-singular/test_pino.png')