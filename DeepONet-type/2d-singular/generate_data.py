...
if __name__ == '__main__':
    ...
    f = np.load('DeepONet-type/2d-singular/saved_data/f.npy')
    ...
    for k in range(ntotal):
        ...
        _, _, ut, _, _ = test_tfpm_refine.tfpm2d(N*M, f_fine_each)
        if k<ntrain:
            ut_train_vertex[k] = ut[::M, ::M]
        else:
            ut_test_fine[k-ntrain] = ut[:]
            f_test_fine[k-ntrain] = f_fine_each[:]
    np.savez("DeepONet-type/2d-singular/saved_data/data.npz", f_total=f_total, B_total=B_total, C_total=C_total, up_total=up_total, index=index, val=val, u_test_fine=ut_test_fine)
    np.savez("DeepONet-type/2d-singular/saved_data/data_fno.npz", u_test_fine=ut_test_fine, u_train_sparse=ut_train_vertex, f_test_fine=f_test_fine)