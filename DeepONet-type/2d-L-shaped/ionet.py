...
f_total = np.load('DeepONet-type/2d-L-shaped/saved_data/f_centor.npy').reshape((ntotal, N, N))
...
for ep in range(epochs):
    ...
    for ff0, l0, y0, ff1, l1, y1 in train_loader:
        ...
        if type_ == 'supervised':
            ...
            Cx1x2 = torch.where(((0.25 <= x01) & (x01 <= 0.5) & (0.25 <= x00) & (x00 <= 0.75)) |
                                ((0.5 <= x01) & (x01 <= 0.75) & (0.25 <= x00) & (x00 <= 0.5)), 1.0, 16.0)
            ...
            Cx1x2 = torch.where(((0.25 <= x11) & (x11 <= 0.5) & (0.25 <= x10) & (x10 <= 0.75)) |
                                ((0.5 <= x11) & (x11 <= 0.75) & (0.25 <= x10) & (x10 <= 0.5)), 1.0, 16.0)
...
torch.save({'model0': model0.state_dict(), 'model1': model1.state_dict()}, 'DeepONet-type/2d-L-shaped/saved_data/{}_ionet_model_state.pth'.format(type_))
