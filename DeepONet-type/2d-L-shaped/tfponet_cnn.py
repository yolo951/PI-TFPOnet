...
class DeepONet2D(nn.Module):
    def __init__(self, b_dim, t_dim):
        ...
        self.b = torch.nn.Parameter(torch.zeros(1, 4))

    def forward(self, x, l):
        x = self.branch(x)
        l = self.trunk(l)

        res = torch.einsum("bij,bki->bkj", x, l)
        res.unsqueeze(-1)
...
modes2 = 12
width = 32
model = DeepONet2D(N**2, 2).to(device)
...
f_centor = np.load('DeepONet-type/2d-L-shaped/saved_data/f_centor.npy')
...
f_train = torch.tensor(f_centor[0:ntrain], dtype=torch.float32).to(device)
...
f_test = torch.tensor(f_centor[ntrain:ntotal], dtype=torch.float32).to(device)
...
train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(f_train, loc_train, B_train, C_train), batch_size=batch_size, shuffle=True)
...
for i in range(epochs):
    model.train()
    train_mse = 0
    for fb, l, Bb, Cb in train_loader:
        optimizer.zero_grad()
        Cb_pred = model(fb, l)
    ...
    if i==0 or (i+1)%100==0:
        ...
        u_pred = model(f_test, loc_test).reshape(ntest, -1, 4).sum(axis=-1)
...
np.save('DeepONet-type/2d-L-shaped/saved_data/tfpo_deeponet_loss.npy', loss_history)
np.save('DeepONet-type/2d-L-shaped/saved_data/tfpo_deeponet_rel_l2.npy', rel_l2_history)
torch.save(model.state_dict(), 'DeepONet-type/2d-L-shaped/saved_data/tfpo_deeponet_model.pt')

C_pred = model(f_train, loc_train).detach().cpu().reshape(f_train.shape[0], -1)
...
C_pred = model(f_test, loc_test).detach().cpu().reshape(f_test.shape[0], -1)
...
for ax in [ax0, ax1, ax2]:
    ax.set_aspect('equal')
plt.savefig('DeepONet-type/2d-L-shaped/saved_data/tfpo_deeponet_compare.png')
...
# plt.ylim(1e-5, 1e+2)
plt.yscale("log")
plt.savefig('DeepONet-type/2d-L-shaped/saved_data/tfpo_deeponet_loss.png')
# plt.show()
...
# plt.ylim(1e-3, 1e+2)
plt.yscale("log")
plt.savefig('DeepONet-type/2d-L-shaped/saved_data/tfpo_deeponet_l2.png')
plt.show()