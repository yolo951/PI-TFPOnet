...
loc_test = loc_total[ntrain:ntotal] 
...
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(f_test,loc_test,up_test), batch_size=batch_size, shuffle=True)
...
for ep in range(epochs):
    ...
    for x, l, y in train_loader:
        ...
        if type_ == 'supervised':
            ...
            F = - eps**2*(y_x1x1 + y_x2x2) + Cx1x2*y_pred_f - x.unsqueeze(2)
...
with torch.no_grad(): 
    ...
    print(torch.linalg.norm(up_test.flatten()).item())
    print('test error on high resolution: relative L2 norm = ', torch.linalg.norm(up_pred.flatten() - up_test.flatten()).item() / torch.linalg.norm(up_test.flatten()).item())
    print('test error on high resolution: relative L2 norm = ', torch.linalg.norm(up_pred.flatten() - up_test.flatten(), ord=torch.inf).item() / torch.linalg.norm(up_test.flatten(), ord=torch.inf).item())
