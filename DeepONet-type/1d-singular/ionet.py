...
scheduler0 = torch.optim.lr_scheduler.StepLR(optimizer0, step_size=300, gamma=0.5)
scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer1, step_size=300, gamma=0.5)
start = default_timer()
for ep in range(epochs):
    ...
    for x0, l0, y0, r0, lb0, x1, l1, y1, r1, lb1, lb_mid in train_loader:
        ...
        if type_ == 'supervised':
            ...
            mse_equ = 20.0*F.mse_loss(-0.001*y_ll0+q1(l0)*out0, r0) + 20.0*F.mse_loss(-0.001*y_ll1+q2(l1)*out1, r1)
            out0, out1 = model0(x0, lb0), model1(x1, lb1)
            mse_b = 2.0*F.mse_loss(out0, torch.zeros_like(out0)) + F.mse_loss(out1, torch.zeros_like(out1))
