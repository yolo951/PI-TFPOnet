...
class PINO_loss:
    def __init__(self, eps, batch_size, N, coeffs, myloss, device):
        ...
        self.q_left = 5.*torch.ones_like(x_left)
        self.q_right = 0.1*(4+32.*x_right)
...
if __name__ == '__main__':
    ...
    pino_loss = PINO_loss(0.001, batch_size, N, coeffs, myloss, device)
    model_left.train()
    model_right.train()
    ...
    N_fine = 256
    f_test_fine = np.load('DeepONet-type/1d-singular/f_test_fine.npy')
    u_test_fine = np.load('DeepONet-type/1d-singular/u_test.npy')
