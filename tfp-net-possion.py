...
class PhysicsInformedNN():
    ...
    def loss_func(self):
        ...
        loss = self.loss(torch.matmul(self.U, AB_pred), self.B)
        loss.backward()
...
f = lambda x: 1
u_exact = lambda x: -0.5*x*x+1.5*x
u, U, B, coeff, AB = discontinuous_tfpm(f, N_x)
# plt.plot(gridx, u.flatten(), label='tfpm')
# plt.plot(gridx, u_exact(gridx), label='exact')
# plt.legend()
# plt.show()
...
# interior_x = gridx[1:]
# interior_U = U[1:-1].reshape((N_x-2, 2, 4))
# interior_B = B[1:-1]
# boundary_x = np.array([gridx[0], gridx[-1]])
# boundary_B = np.array([B[0], B[-1]])
# boundary_U = np.stack((U[0, :2], U[-1, -2:]))
...
plt.plot(gridx, prediction, 'b-', label='prediction', alpha=1., zorder=0)
plt.legend(loc='best')
plt.show()

fig = plt.figure(figsize=(7, 3), dpi=150)
plt.subplot(1, 2, 1)
plt.plot(gridx, u_exact(gridx), 'r-', label='Ground Truth', alpha=1., zorder=0)
plt.plot(gridx, prediction, 'b-', label='prediction', alpha=1., zorder=0)
plt.legend(loc='best')
plt.xlabel("x")
plt.ylabel("$u$")
